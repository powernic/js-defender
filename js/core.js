...
var Core = new function(){
	...
	var ORGANISM_ENEMY_ITEM,
		ORGANISM_ENERGY_ITEM,
		PLAYER_ITEM;
	...
	this.init = function(){
		...
		if (canvas && canvas.getContext) {
			...
			var loaded = setInterval(function(){
				if( typeof ORGANISM_ENEMY_ITEM === 'object'){
					animate();
			...
			setInterval(function(){
				player.pulseOffset = player.pulseOffset === 0 ? 2 : 0;
			},300);
		}
	};
	...
	function setOrganismItems() {
		var organismEnemyImg = new Image();
...
*/
		...
		for( i = 0; i < organisms.length; i++ ) {
			...
			if( p.type == ORGANISM_ENEMY ) {
				organism = ORGANISM_ENEMY_ITEM;
			};
			if( p.type == ORGANISM_ENERGY ) {
				organism = ORGANISM_ENERGY_ITEM;
			};
		...
		for( i = 0; i < particles.length; i++ ) {
			...
			// Draw the particle
			context.fillStyle = 'rgba(255,185,0,'+Math.max(p.alpha,0)+')';
